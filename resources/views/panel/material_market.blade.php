...
<div class="flex flex-col md:flex-row justify-between items-end mb-8 gap-4 border-b-2 border-dashed border-black pb-6">
    <div>
        <h1 class="text-4xl font-black text-gray-900 tracking-tighter uppercase italic">Material Market</h1>
        <p class="mt-2 font-bold text-gray-500 font-mono">Manage special material exchanges.</p>
    </div>
    <button onclick="openAddModal()" 
        class="bg-[#EC4899] text-white px-6 py-3 font-bold border-2 border-black shadow-neo hover:shadow-none hover:translate-x-[4px] hover:translate-y-[4px] transition-all flex items-center gap-2 uppercase">
...
<div class="bg-white border-2 border-black shadow-neo">
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            <thead class="text-sm font-black uppercase text-black bg-[#FBCFE8] border-b-2 border-black">
                <tr>
                    <th class="px-6 py-4 border-r-2 border-black">Target Material</th>
                    <th class="px-6 py-4 border-r-2 border-black">Category</th>
                    <th class="px-6 py-4 border-r-2 border-black">Cost Requirements</th>
            ...
            <tbody class="divide-y-2 divide-black">
                ...
                @endphp
                <tr class="hover:bg-[#FDF2F8] transition-colors font-medium">
                    ...
                    <td class="px-6 py-4 text-right">
                         <div class="flex justify-end gap-2">
                            ...
                            <form action="{{ route('panel.material_market.delete', $item->id) }}" method="POST" onsubmit="return confirm('Delete this exchange?');">
                                @csrf
                                @method('DELETE')
...
<div id="addModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="flex items-center justify-center min-h-screen p-4 text-center">
        <div class="relative bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-left transform transition-all sm:max-w-xl w-full p-0">
             <form action="{{ route('panel.material_market.add') }}" method="POST" onsubmit="prepareSubmission('add')">
                @csrf
                <div class="bg-[#EC4899] px-6 py-5 border-b-2 border-black flex justify-between items-center">
                    <h3 class="text-2xl font-black text-white italic tracking-tighter uppercase">NEW EXCHANGE</h3>
                    <button type="button" onclick="closeAddModal()" class="text-white hover:text-black transition-colors">
                ...
                <div class="p-8 bg-white space-y-6">
                    <!-- Target Selector -->
                    <div class="relative">
                        <label class="block text-sm font-black uppercase text-gray-900 mb-2 pl-2 border-l-4 border-black">Target Material ID</label>
                        <input type="text" id="add_target_search" placeholder="Search Item or Skill..." 
                            class="block w-full text-sm font-bold border-2 border-black px-4 py-3 focus:ring-0 focus:shadow-[4px_4px_0px_0px_rgba(236,72,153,1)] transition-all" autocomplete="off">
                    ...
                    <div>
                        <label class="block text-sm font-black uppercase text-gray-900 mb-2 pl-2 border-l-4 border-black">Category</label>
                         <select name="category" id="add_category" class="block w-full text-sm font-bold border-2 border-black px-4 py-3 focus:ring-0 focus:shadow-[4px_4px_0px_0px_rgba(236,72,153,1)] transition-all uppercase">
                            <option value="material">Material</option>
                            <option value="skill">Skill</option>
                            <option value="wpn">Weapon</option>
                    ...
                    <!-- Cost Requirements Builder -->
                    <div>
                        <label class="block text-sm font-black uppercase text-gray-900 mb-2 pl-2 border-l-4 border-black">Cost Requirements</label>
                ...
                <div class="bg-gray-50 px-6 py-5 border-t-2 border-black flex flex-row-reverse gap-4">
                    <button type="submit" class="w-full sm:w-auto bg-[#EC4899] text-white px-6 py-3 font-bold border-2 border-black shadow-neo hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all uppercase">Create</button>
...
<div id="editModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="flex items-center justify-center min-h-screen p-4 text-center">
        <div class="relative bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-left transform transition-all sm:max-w-xl w-full p-0">
             <form action="{{ route('panel.material_market.update') }}" method="POST" onsubmit="prepareSubmission('edit')">
                @csrf
                ...
                <div class="p-8 bg-white space-y-6">
                    <div>
                        ...
                        <!-- Keep original logic for category if needed, but usually we don't change category on edit? Actually we can -->
                    </div>
                     <div>
                        <label class="block text-sm font-black uppercase text-gray-900 mb-2 pl-2 border-l-4 border-black">Category</label>
                         <select name="category" id="edit_category" class="block w-full text-sm font-bold border-2 border-black px-4 py-3 focus:ring-0 focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all uppercase">
                            <option value="material">Material</option>
                            <option value="magatama">Magatama</option>
                            <option value="skill">Skill</option>
                            <option value="wpn">Weapon</option>
                            <option value="set">Armor Set</option>
                            <option value="hair">Hair</option>
                    ...
                    <!-- Cost Requirements Builder (Edit) -->
                    <div>
                        <label class="block text-sm font-black uppercase text-gray-900 mb-2 pl-2 border-l-4 border-black">Cost Requirements</label>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        setupSearch('add_target_search', 'add_target_list', combinedData, (obj) => {
            ...
            if (obj._type === 'skill') {
                ...
                } else if(obj.type === 'magatama') { // Assuming magatama might be a type
                    catSelect.value = 'magatama';
                } else {
                    // Try to match directly (wpn, set, hair, back, accessory)
                    catSelect.value = obj.type;
                }
            } else {
                catSelect.value = 'material';
            }
    ...
    // Requirements Logic
    function addRequirement(mode) { // mode: 'add' or 'edit'
        const inputId = document.getElementById(`${mode}_req_temp_id`).value || document.getElementById(`${mode}_req_search`).value; 
        // Fallback to text input if user typed ID directly? 
        // Better to rely on selection, but allow ID input if valid.
        // For now assume selection or direct ID type if matches.
        
        // Actually, if temp_id is empty, user might have typed ID manually?
        // Let's stick to what's in search box if temp_id empty, but warn?
        // Let's use search box value as ID if temp_id is empty (user typed "1001")
    ...
    function renderRequirements(mode) {
        ...
        requirementsMap[mode].forEach((req, idx) => {
            // Find name if we only have ID (for Edit mode init)
            let name = req.name;
            if(!name || name === req.id) {
    ...
    function prepareSubmission(mode) {
        // Serialize requirements to "id:qty, id:qty"
    ...
    function openEditModal(id, itemId, reqString, category) {
        ...
        // Parse requirements string: "1001:5, 1002:10"
            ...
            parts.forEach(p => {
                ...
                if(pid) {
                    requirementsMap['edit'].push({
                        ...
                        name: pid.trim() // Will be resolved in render
                    });
                }
            });
        }
