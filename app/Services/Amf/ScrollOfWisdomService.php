...
class ScrollOfWisdomService
{
        ...
        try {
            return DB::transaction(function () use ($charId, $sessionKey, $elementType) {
                ...
                foreach ($skillsToAdd as $skillId) {
                    // Check if this skill belongs to a chain
                    if (isset($skillToGroupSkills[$skillId])) {
                        $groupSkills = $skillToGroupSkills[$skillId];

                        // Find any existing skills from this chain that are NOT the new skill
                        // (i.e., lower or different versions)
                    ...
                    // Finally, ensure the new skill exists
    ...
    {
        // 1: Wind, 2: Fire, 3: Lightning, 4: Earth, 5: Water
        
        $rawSkills = \App\Helpers\ElementSkillHelper::getSkillsForElement($type);
        
        // Get Chains from AdvanceAcademyService
        $advanceService = app(\App\Services\Amf\AdvanceAcademyService::class);
        ...
        // Map skill ID -> Chain Info (Group Name, Index in Chain)
        $skillChainMap = [];
        ...
        // Filter: Keep only the highest index skill for each chain group found in $rawSkills
        $highestInGroup = [];
        $finalSkills = [];
        ...
        foreach ($rawSkills as $rSkill) {
            if (isset($skillChainMap[$rSkill])) {
                ...
                // Not in a chain, keep it
                $finalSkills[] = $rSkill;
            }
        }

        // Add the highest chain skills
